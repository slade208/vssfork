...
async def summarize(
    ...
    if gr_video is None:
        ...
        gradio_path = f"/tmp/gradio/{request_id}.mp4"
        cached_path = f"/tmp/via/cached_frames/{request_id}/{request_id}.mp4"
        gradio_done = f"/tmp/gradio/{request_id}.done"
        cached_done = f"/tmp/via/cached_frames/{request_id}/overlay.done"
        def resolve_overlay_video_path():
            # Only expose overlay after ffmpeg finalize marker exists.
            if (
                os.path.exists(gradio_done)
                and os.path.exists(gradio_path)
                and os.path.getsize(gradio_path) > 0
            ):
                return gradio_path
            if (
                os.path.exists(cached_done)
                and os.path.exists(cached_path)
                and os.path.getsize(cached_path) > 0
            ):
                try:
                    shutil.copyfile(cached_path, gradio_path)
                except Exception:
                    pass
                if (
                    os.path.exists(gradio_done)
                    ...
                    return gradio_path
            return None
        overlay_video_path = resolve_overlay_video_path()

        overlay_title = (
            "<b>Here is the CV overlay video</b>"
            if enable_cv_metadata
            else "<b>Here is the preprocessed set-of-marks video</b>"
        )
        logger.info(
            "Overlay UI: request_id=%s enable_cv_metadata=%s overlay_video_path=%s",
            request_id,
            enable_cv_metadata,
            overlay_video_path,
        )
        if overlay_video_path:
            out_vid = (
                overlay_title
                + """<div style=\"max-width: 600px !important;min-width: 600px !important;\">"""
                + f"""<video src=\"./gradio_api/file=/tmp/gradio/{request_id}.mp4\" autoplay controls inline  /> """
                + """</div>"""
            )
            logger.info("Overlay UI: rendering video block for %s", request_id)
        else:
            out_vid = (
                overlay_title
                + """<div style=\"max-width: 600px !important;min-width: 600px !important;\">"""
                + """<div>Building overlay video... 0s</div>"""
                + """</div>"""
            )
            logger.info("Overlay UI: rendering placeholder for %s", request_id)
        chatbot += [[None, out_vid]]
        # await remove_all_media(session, media_ids)

        if parsed_alerts and not past_alerts:
            output_alerts = "No alerts were generated for this input media"

        video_update = gr.update(interactive=True)
        ...
        if enable_cv_metadata and not overlay_video_path:
            wait_sec = int(os.getenv("VSS_CV_OVERLAY_WAIT_SEC", "600"))
            poll_sec = max(int(os.getenv("VSS_CV_OVERLAY_POLL_SEC", "2")), 1)
            start_wait = time.time()
            deadline = start_wait + max(wait_sec, 0)
            logger.info("Overlay UI: waiting up to %ss for overlay video %s", wait_sec, request_id)
            ...
            while time.time() <= deadline:
                ...
                if overlay_video_path:
                    ...
                    chatbot[-1] = [None, out_vid]
                    logger.info(
                        "Overlay UI: rendering video block after wait for %s (elapsed=%ss)",
                        request_id,
                        elapsed,
                    )
                    yield (
                ...
                chatbot[-1] = [
                    ...
                    overlay_title
                    + """<div style=\"max-width: 600px !important;min-width: 600px !important;\">"""
                    + f"""<div>Building overlay video... {elapsed}s</div>"""
                    + """</div>""",
                ]
                yield (
                    chatbot,
            ...
            if not overlay_video_path:
                ...
                chatbot[-1] = [
                    ...
                    overlay_title
                    + """<div style=\"max-width: 600px !important;min-width: 600px !important;\">"""
                    + f"""<div>Overlay video is still encoding in background ({elapsed}s).</div>"""
                    + """</div>""",
                ]
                logger.info(
                    "Overlay UI: overlay still not ready after wait for %s (elapsed=%ss)",
                    request_id,
                    elapsed,
                )
                yield (
...
def get_display_image(f, image_mode):
    if image_mode:
        ...
        # Use the packaged asset path instead of a hard-coded container path.
        return CHATBOT_AVATAR_ICON.name
...
def build_summarization(args, app_cfg, logger_):
    ...
    def on_select_example(selected_media, stream_full_path, image_mode):
        if not stream_full_path:
            return (gr.update(), gr.update(), gr.update())

        if image_mode:
            selected_media = selected_media or []
            return (
                (selected_media + [stream_full_path]),
                gr.update(interactive=True, value="Summarize"),
                gr.update(value="Ask"),
            )
    ...
    # Use the stream path change as the trigger so samples still switch
    # when the thumbnail icon is the same for multiple examples.
    stream_full_path.change(
        on_select_example,
        inputs=[video, stream_full_path, gr.State(args.image_mode)],
