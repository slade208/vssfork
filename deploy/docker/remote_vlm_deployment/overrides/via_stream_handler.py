...
class ViaStreamHandler:
    ...
    def __init__(self, args) -> None:
        ...
        self._ctx_mgr_pool = []
        # Allow runtime tuning to avoid rate-limits on remote LLMs.
        self.NUM_CA_RAG_PROCESSES_LAUNCH = int(
            os.getenv("NUM_CA_RAG_PROCESSES_LAUNCH", "10")
        )
        self.num_ctx_mgr = 0
        ...
        if not args.disable_ca_rag:
            try:
                ...
                # Force Milvus connection params from env if provided.
                milvus_host = (
                    os.environ.get("MILVUS_DB_HOST")
                    or os.environ.get("MILVUS_HOST")
                    or os.environ.get("MILVUS_ADDRESS")
                )
                milvus_port = (
                    os.environ.get("MILVUS_DB_GRPC_PORT")
                    or os.environ.get("MILVUS_DB_PORT")
                    or os.environ.get("MILVUS_PORT")
                )
                if milvus_host or milvus_port:
                    config.setdefault("tools", {}).setdefault("vector_db", {}).setdefault(
                        "params", {}
                    )
                    if milvus_host:
                        config["tools"]["vector_db"]["params"]["host"] = str(milvus_host)
                    if milvus_port:
                        config["tools"]["vector_db"]["params"]["port"] = str(milvus_port)
                    # Ensure pymilvus default URI matches overrides to avoid localhost fallback.
                    if milvus_host and milvus_port:
                        os.environ["MILVUS_URI"] = f"http://{milvus_host}:{milvus_port}"
                    logger.info(
                        "Milvus override: host=%s port=%s",
                        config["tools"]["vector_db"]["params"].get("host"),
                        config["tools"]["vector_db"]["params"].get("port"),
                    )
    ...
    def _create_video_from_cached_frames(self, req_info):
        ...
        gradio_video_path = f"/tmp/gradio/{req_info.request_id}.mp4"
        images_path = f"{cached_frames_dir}/frame_*.jpg"
        ffmpeg_path = shutil.which("ffmpeg_for_overlay_video")
        logger.info(
            "Attempting ffmpeg overlay build. dir=%s video_path=%s ffmpeg=%s",
            cached_frames_dir,
            video_path,
            ffmpeg_path,
        )
        for path in [video_path, gradio_video_path]:
            if os.path.exists(path):
                if os.path.getsize(path) > 0:
                    return path
                logger.warning("Overlay video exists but is empty: %s", path)
                try:
                    os.remove(path)
                except Exception:
                    pass
        ...
        if os.path.exists(cached_frames_dir) and has_ffmpeg:
            logger.info("Scanning for overlay frames in %s", cached_frames_dir)
            try:
                frame_names = [
                    entry.name
                    for entry in os.scandir(cached_frames_dir)
                    if entry.is_file()
                    and entry.name.startswith("frame_")
                    and entry.name.endswith(".jpg")
                ]
            except Exception as e:
                logger.error("Failed to scan frames: %s", e)
                return None
            if not frame_names:
                logger.warning(
                    "No cached frames found; skipping overlay video creation. dir=%s",
                    cached_frames_dir,
                )
                return None
            frame_names.sort()

            fps = float(os.getenv("VSS_CV_OVERLAY_FPS", "30"))
            if fps <= 0:
                fps = 30.0

            logger.info(
                "Creating cached frames video at %.2f fps",
                fps,
            )
            list_path = os.path.join(cached_frames_dir, "frames.txt")
            try:
                logger.info("Writing ffmpeg concat list with %d frames", len(frame_names))
                with open(list_path, "w") as f:
                    for name in frame_names:
                        f.write(f"file '{os.path.join(cached_frames_dir, name)}'\n")
                logger.info("Wrote ffmpeg concat list: %s", list_path)
            except Exception as e:
                logger.error("Failed to write ffmpeg list file: %s", e)
                return None
            # Write to gradio tmp to avoid cached_frames cleanup races; copy back on success.
            command = [
            ...
            try:
                timeout_sec = int(os.getenv("VSS_CV_OVERLAY_FFMPEG_TIMEOUT_SEC", "600"))
                result = subprocess.run(
                    command, capture_output=True, text=True, check=True, timeout=timeout_sec
                )
                if result.stdout:
                    logger.info("ffmpeg stdout: %s", result.stdout.strip())
                if result.stderr:
                    logger.info("ffmpeg stderr: %s", result.stderr.strip())
                if os.path.exists(gradio_video_path):
                    try:
                        shutil.copyfile(gradio_video_path, video_path)
                    except Exception as e:
                        logger.warning("Failed to copy overlay video to cache: %s", e)
                logger.info("Cached Frames Video created at %s", gradio_video_path)

                done_path = os.path.join(cached_frames_dir, "overlay.done")
                gradio_done_path = f"/tmp/gradio/{req_info.request_id}.done"
                try:
                    Path(done_path).write_text("ok")
                except Exception:
                    pass
                try:
                    Path(gradio_done_path).write_text("ok")
                except Exception:
                    pass
                ...
                logger.error("FFmpeg overlay build timed out after %ds", timeout_sec)
                return None
            except subprocess.CalledProcessError as e:
                logger.error(
                    "FFmpeg command failed: %s | stdout=%s | stderr=%s",
                    e,
                    getattr(e, "stdout", ""),
                    getattr(e, "stderr", ""),
                )
    ...
    def _render_overlay_video_from_cv(self, req_info):
        if not req_info.enable_cv_pipeline or not req_info.cv_metadata_json_file:
            return None
        if req_info.file.startswith("rtsp://"):
            return None
        if os.getenv("VSS_CV_RENDER_OVERLAY_VIDEO", "true").lower() not in [
            "1",
            "true",
            "yes",
        ]:
        ...
        overlay_dir = f"/tmp/via/cached_frames/{req_info.request_id}"
        overlay_path = f"{overlay_dir}/{req_info.request_id}.mp4"
        gradio_path = f"/tmp/gradio/{req_info.request_id}.mp4"
        if os.path.exists(gradio_path) and os.path.getsize(gradio_path) > 0:
            return gradio_path
        ...
        try:
            ...
            logger.error("Failed to create overlay directory %s: %s", overlay_dir, ex)
            return None

        direct_render_enabled = os.getenv("VSS_CV_OVERLAY_DIRECT_RENDER", "false").lower() in [
            "1",
            "true",
            "yes",
        ]
        direct_render_timed_out = False
        if direct_render_enabled:
            try:
                logger.info(
                    "Starting overlay render for %s using %s",
                    req_info.request_id,
                    req_info.cv_metadata_json_file,
                )
                ...
                def _worker():
                    try:
                        fgetter.render_overlay_video(
                            req_info.file, req_info.cv_metadata_json_file, req_info.request_id
                        )
                    finally:
                        done.set()
                ...
                timeout_sec = float(os.getenv("VSS_CV_OVERLAY_RENDER_TIMEOUT_SEC", "180"))
                if timeout_sec > 0:
                ...
                if not done.is_set():
                    direct_render_timed_out = True
                    logger.error(
                        "Overlay render timed out after %.1fs for %s; terminating pipeline.",
                        timeout_sec,
                        req_info.request_id,
                    )
                    fgetter.dispose_pipeline_from_separate_thread()
                    # Give disposal a moment to complete.
                    t.join(timeout=5)
            except Exception as ex:
                logger.error("Overlay video render failed for %s: %s", req_info.request_id, ex)
                return None
        else:
            logger.info(
                "Direct overlay render disabled for %s; using frame-dump + ffmpeg path.",
                req_info.request_id,
            )

        if not direct_render_timed_out:
            if os.path.exists(overlay_path):
                logger.info("Overlay render complete for %s: %s", req_info.request_id, overlay_path)
            else:
                logger.error(
                    "Overlay render finished for %s but output file missing: %s",
                    req_info.request_id,
                    overlay_path,
                )
            if os.path.exists(overlay_path) and os.path.getsize(overlay_path) > 0:
        ...
        # Fallback: dump overlay frames and build mp4 via ffmpeg.
        logger.warning(
            "Overlay video missing or empty for %s; falling back to frame-dump + ffmpeg.",
            req_info.request_id,
        )
        fallback_fgetter = None
        try:
            fallback_fgetter = VideoFileFrameGetter(
                frame_selector=DefaultFrameSelector(8),
                frame_width=0,
                frame_height=0,
                gpu_id=0,
            ...
            fallback_fgetter.render_overlay_video_frames(
                req_info.file, req_info.cv_metadata_json_file, req_info.request_id
            )
            overlay_path = self._create_video_from_cached_frames(req_info)
            if overlay_path and os.path.exists(overlay_path) and os.path.getsize(overlay_path) > 0:
                logger.info(
                    "Overlay video created via ffmpeg for %s: %s",
                    req_info.request_id,
                    overlay_path,
                )
                return overlay_path
            ...
            logger.error(
                "Overlay video fallback failed for %s: %s", req_info.request_id, ex
            )
        finally:
            try:
                fallback_fgetter.dispose_pipeline_from_separate_thread()
            except Exception:
                pass
        return None
    ...
    def _on_vlm_chunk_response(self, response: VlmChunkResponse, req_info: RequestInfo):
        ...
        if response.error:
            ...
            if req_info._ctx_mgr:
                ...
                if (
                    not cv_meta
                    and chunk.cv_metadata_json_file
                    and hasattr(response, "frame_times")
                    and response.frame_times
                ):
                    cv_meta = self._get_cv_metadata_for_chunk(
                        chunk.cv_metadata_json_file, response.frame_times
                    )
                    chunk.cached_frames_cv_meta = cv_meta
    ...
    def query(
        ...
        if self._cv_pipeline and req_info.enable_cv_pipeline:
            ...
            def _on_cv_pipeline_done(json_fused_file, req_info):
                ...
                # Render full overlay video from CV metadata (no sampled-frame stutter)
                threading.Thread(
                    target=self._render_overlay_video_from_cv,
                    args=(req_info,),
                    daemon=True,
                ).start()
                ...
                try:
                    with nvtx.annotate(
                        ...
                        if req_info.summarize:
                            ...
                            if isinstance(agg_response, dict) and "summarization" in agg_response:
